...
from util_scripts.inn import *
from util_scripts.dataset import DataType, Dataset
...
import time
from util_scripts.optsolver import OptSolver
...
class UtilExp:
    ...
    def is_robust_custom_delta_new(self, cf, delta):
        nodes = build_inn_nodes(self.clf, self.num_layers)
        weights, biases = build_inn_weights_biases(self.clf, self.num_layers, delta, nodes)
        inn = Inn(self.num_layers, delta, nodes, weights, biases)
        y_prime = 1
        this_solver = OptSolver(self.dataset, inn, y_prime, None, mode=1, M=10000, x_prime=cf)
        found, bound = this_solver.compute_inn_bounds()
        #lmarza
        return bound
        #if found == 1:
    ...
    def run_ours_robust(self, approx=False):
        return self.run_ours(robust=True, delta=None, approx=approx)

    def run_ours_non_robust(self):
        return self.run_ours(robust=False, delta=None)

    def run_ours(self, robust=False, delta=None, approx=False):
        ...
        for i, x in tqdm(enumerate(self.test_instances)):
            if robust is False:
                ...
                this_cf = self.run_ours_one_delta_robust(x, delta=delta, approx=approx)
            CEs.append(this_cf)
    ...
    def estimate_robustness(self, model, delta, cfx, concretizations=10000, use_biases=True, robustness=True):
    ...
    def run_ours_one_delta_robust(self, x, delta=None, approx=False):
        y_prime = 1 if self.clf.predict(x.reshape(1, -1))[0] == 0 else 0
        # tolerance for the robustness (i.e., pre-activated lower-bound < eps)
        eps = 0.01
        this_solver = OptSolver(self.dataset, self.inn_delta_0, y_prime, x, mode=0, eps=0.01, x_prime=None)
        this_cf = this_solver.compute_counterfactual()
        count = 0
        if approx: delta=self.delta_min
        if delta is None:  # default, delta=self.delta_min
            ...
            # we want the safer CFX as possible 
            # here we check the CFX for a specific delta
            # we just need to replace self.is_robust_custom_delta(x, this_cf, delta=delta) with Wilks
            if approx:
                input_size, n_layers, output_size, output_act, h_act, optimizer, params = extract_sklearn_params(self.clf)
                model = custom_nn_model(input_size, n_layers, params, output_size, h_act, output_act, optimizer)
            
                # Set model weights
                for k, v in params.items():
                    model.layers[k].set_weights(v)
                while self.estimate_robustness(model, delta, this_cf) != 1 and this_cf is not None:
                #while self.is_robust_custom_delta(x, this_cf, delta=delta) != 1 and this_cf is not None:
                    this_solver = OptSolver(self.dataset, self.inn_delta_0, y_prime, x, mode=0, eps=eps, x_prime=None)
                    this_cf = this_solver.compute_counterfactual()
                    # if not robust we increment eps 
                    eps += 0.2
            else:
                while self.is_robust_custom_delta(x, this_cf, delta=delta) != 1 and this_cf is not None:
                    this_solver = OptSolver(self.dataset, self.inn_delta_0, y_prime, x, mode=0, eps=eps, x_prime=None)
